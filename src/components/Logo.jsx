...
const Logo = ({ small, font_color = '#888b8d' }) => {
    // eslint-disable-next-line no-undef
    const { innerWidth } = window;
    const width = innerWidth === 768 ? 20 : 145;

    const largeHeader = largeHeaderData(font_color);

    const viewBox = innerWidth === 768 ? '0 0 150 145' : '0 0 720 145';
    if (!small) {
        headerData.splice(0, 0, largeHeader[0]);
        headerData.splice(1, 0, largeHeader[1]);
        headerData.splice(2, 0, largeHeader[2]);
    }
    return (
        <svg
            style={{ marginTop: '5px' }}
